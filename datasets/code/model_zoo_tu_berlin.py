...
def initialize_model(model_name, num_classes, feature_extract, use_pretrained=True):
    ...
    if model_name == "densenet121":
        ...
        model_ft.fc = nn.Linear(num_ftrs, 250)# tu-berlin dataset num class
        PATH = '/data/jongmin/repos/asd_project/result_folder/finetune_folder/tu_berlin_saved_model_epoch100/{}.pt'.format(model_name)
        model_ft.load_state_dict(torch.load(PATH), strict=False)
        set_parameter_requires_grad(model_ft, feature_extract)
        num_ftrs = model_ft.fc.in_features
        ...
        set_parameter_requires_grad(model_ft, feature_extract)
        num_ftrs = model_ft.fc.in_features
        model_ft.fc = nn.Linear(num_ftrs, 251)# tu-berlin dataset num class
        PATH = '/data/jongmin/repos/asd_project/tu_berlin/tu_berlin_saved_model/{}.pt'.format(model_name)
        model_ft.load_state_dict(torch.load(PATH), strict=False)
        set_parameter_requires_grad(model_ft, feature_extract)
        num_ftrs = model_ft.fc.in_features
        model_ft.fc = nn.Linear(num_ftrs, num_classes)
        ...
        model_ft.classifier[3] = nn.Linear(num_ftrs, 251)# tu-berlin dataset num class
        PATH = '/data/jongmin/repos/asd_project/tu_berlin/tu_berlin_saved_model/{}.pt'.format(model_name)
        model_ft.load_state_dict(torch.load(PATH), strict=False)
        set_parameter_requires_grad(model_ft, feature_extract)
        num_ftrs = model_ft.classifier[3].in_features
        ...
        model_ft.heads.head = nn.Linear(num_ftrs, 251)
        PATH = '/data/jongmin/repos/asd_project/tu_berlin/tu_berlin_saved_model/{}.pt'.format(model_name)
        model_ft.load_state_dict(torch.load(PATH), strict=False)
        set_parameter_requires_grad(model_ft, feature_extract)
        num_ftrs = model_ft.heads.head.in_features
        ...
        set_parameter_requires_grad(model_ft, feature_extract)
        num_ftrs = model_ft.heads.head.in_features
        model_ft.heads.head = nn.Linear(num_ftrs, 250)
        PATH = '/data/jongmin/repos/asd_project/result_folder/finetune_folder/tu_berlin_saved_model_epoch100/{}.pt'.format(model_name)
        model_ft.load_state_dict(torch.load(PATH), strict=False)
        set_parameter_requires_grad(model_ft, feature_extract)
        num_ftrs = model_ft.heads.head.in_features
