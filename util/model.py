...
class EfficientNet(nn.Module):
    ...
    def __init__(self, depth, pretrained=True, cut_at_pooling=False,
                 num_features=0, norm=False, dropout=0, num_classes=0, FCN=True, radius=1., thresh=0.5):
...
#=======================FCN===============================#
        if self.FCN:
            ...
            sx = x.size(2)/3
            kx = x.size(2)-sx*2
            x = F.avg_pool2d(x,kernel_size=(int(kx),x.size(3)),stride=(int(sx),x.size(3)))   # kernel size -> 2,4
#========================================================================# -> B, C, 3, 1          

            out0 = x.view(x.size(0),-1)
            out0 = x/x.norm(2,1).unsqueeze(1).expand_as(x)
            x = self.drop(x)
            x = self.local_conv(x)# 차원만 1280-> 256해주는 1by 1 convolution
...
def resnet18_pcb(**kwargs):
    return ResNet(18, **kwargs)


def resnet34_pcb(**kwargs):
